...
async function bootstrap() {
  try {
    ...
    // Administrative Route to reactivate a user for testing purposes
    app.post('/api/admin/users/:id/reactivate', async (req, res) => {
      try {
        ...
        res.json({ message: `User ${user.username} reactivated.`, user });
      } catch (err) {
        res.status(500).json({ error: err.message });
      }
    });
    ...
    app.post('/api/auth/refresh', async (req, res) => {
      ...
      const sessionStatus = await redisClient.get(`session:${token}`);
      if (!sessionStatus) {
